...
<div class="publicated texted content">
    ...
    @endif
       @if(!empty($pr->publication->first()->number))<div class="pnamber"><span class="number">№{{$pr->publication->first()->number}}</span> <span class="year">{{$pr->publication->first()->year}}</span></div>@endif
    @endif
</div>
@endif